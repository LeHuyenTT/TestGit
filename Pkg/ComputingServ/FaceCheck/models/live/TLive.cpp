...
void TLive::LoadModel(void) {
    ...
    for (int i = 0; i < model_num_; ++i) {
        ...
        std::string param = "../Pkg/FaceCheck/models/live/" + configs_[i].name + ".param";
        std::string model = "../Pkg/FaceCheck/models/live/" + configs_[i].name + ".bin";
...
cv::Rect TLive::CalculateBox(LiveFaceBox &box, int w, int h, ModelConfig &config) {
    ...
    float scale =
        std::min(config.scale, std::min((w - 1) / (float)box_width, (h - 1) / (float)box_height));
