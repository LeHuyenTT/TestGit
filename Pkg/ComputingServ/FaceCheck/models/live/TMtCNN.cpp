...
TMtCNN::TMtCNN(void) {
    ...
    Pnet.load_param("../Pkg/FaceCheck/models/mtcnn/det1.param");
    Pnet.load_model("../Pkg/FaceCheck/models/mtcnn/det1.bin");
    Rnet.opt.use_vulkan_compute = true;
    Rnet.load_param("../Pkg/FaceCheck/models/mtcnn/det2.param");
    Rnet.load_model("../Pkg/FaceCheck/models/mtcnn/det2.bin");
    Onet.opt.use_vulkan_compute = true;
    Onet.load_param("../Pkg/FaceCheck/models/mtcnn/det3.param");
    Onet.load_model("../Pkg/FaceCheck/models/mtcnn/det3.bin");
}
...
void TMtCNN::generateBbox(ncnn::Mat score, ncnn::Mat location, std::vector<Bbox> &boundingBox_,
                          float scale) {
    const int stride   = 2;
...
void TMtCNN::nmsTwoBoxs(vector<Bbox> &boundingBox_, vector<Bbox> &previousBox_,
...
void TMtCNN::RNet() {
    ...
    for (vector<Bbox>::iterator it = firstBbox_.begin(); it != firstBbox_.end(); it++) {
        ...
        if ((float)score[1] > threshold[1]) {
            ...
            it->area = (it->x2 - it->x1) * (it->y2 - it->y1);
            //			it->score =
            // score.channel(1)[0];//*(score.data+score.cstep);
    ...
    for (vector<Bbox>::iterator it = secondBbox_.begin(); it != secondBbox_.end(); it++) {
        ...
        if ((float)score[1] > threshold[2]) {
            ...
            for (int num = 0; num < 5; num++) {
                ...
                //(it->ppoint)[num + 5] = it->y1 + (it->y2 - it->y1) * keyPoint[num +
                // 5];
